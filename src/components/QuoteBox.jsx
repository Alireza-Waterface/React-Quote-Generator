const QuoteBox = ({
	...
	return (
		<div className="quote">
			{ isLoading ? (
					<div className="quote-box loading">Loading...</div>
				) : error ? (
					<div className="quote-box error">{error}</div>
				) : (
					<div className="quote-box">
						<div className='quote-content'>
							...
							<div className="timer">
								<p className={`countdown ${red ? 'red' : ''}`}>Next quote in: {countdown} seconds</p>
						...
						<div className="share">
							<button className='share-btn bookmark' onClick={handleSaveQuote} title={saved ? 'Remove from saved' : 'Save quote'}>
								{
									saved ?
							...
							<button className="share-btn clipboard" onClick={() => copyToClipboard(quote, author, setAlert)} title='Copy to Clipboard'>
								<svg xmlns="http://www.w3.org/2000/svg" height="1em" viewBox="0 0 384 512"><path d="M192 0c-41.8 0-77.4 26.7-90.5 64H64C28.7 64 0 92.7 0 128V448c0 35.3 28.7 64 64 64H320c35.3 0 64-28.7 64-64V128c0-35.3-28.7-64-64-64H282.5C269.4 26.7 233.8 0 192 0zm0 64a32 32 0 1 1 0 64 32 32 0 1 1 0-64zM112 192H272c8.8 0 16 7.2 16 16s-7.2 16-16 16H112c-8.8 0-16-7.2-16-16s7.2-16 16-16z"/></svg>
							</button>
							<button className="share-btn telegram" onClick={() => shareOnTelegram(quote, author)} title='Share on Telegram'>
							...
							<button className="share-btn whatsapp" onClick={() => shareOnWhatsApp(quote, author)} title='Share on Whatsapp'>
						...
						<input ref={textInputRef} className="hidden-input" type="text" readOnly />
					</div>
				)
			}
			{ !isLoading && !error &&
				<button className="btn-grad generate-btn" onClick={handleFetchNewQuote}>
					Generate new quote
				</button>
			}
			{
				...
				<button className='btn-grad show-saved-btn' onClick={() => setShowSavedQuotes(!showSavedQuotes)}>
					{
						showSavedQuotes ? '' : 'Show Bookmarked'
					}
				</button>
			}
		</div>
